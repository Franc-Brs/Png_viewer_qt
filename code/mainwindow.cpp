    ...
    , ui(new Ui::MainWindow)
{
    ...
    //store new image (nimage) - control this part
    //QImage nimage;

    //setting status of checkbox
...
void MainWindow::cambia_immagine(QString nome_file)
{
    ...
    //changing the format of the image that will be displayed
    c_image = nimage.convertToFormat(QImage::Format_RGB32);
...
void MainWindow::mousePressEvent(QMouseEvent *event)
{
    if(event->button() == Qt::LeftButton)
    {
        ...
        QRgb rgbValue = c_image.pixel(pos_x, pos_y);
...
void MainWindow::on_checkBox_R_stateChanged(int arg1)
{
    if (!arg1)
    {
        //brutal, raw and tailored but it works
        QString col_r="red_d";
        manage_cband(col_r,"","");
    } else {
        QString col_r="";
...
{
    //control on the checkbox, I should re-write ::on_checkBox_X_stateChanged
    QList<QCheckBox *> allckbox = ui->groupBox->findChildren<QCheckBox *>();

    // a new image with the same size
    //QImage c_image(nimage.size(), QImage::Format_RGB32);
    ...
    for (int row = 0; row < nimage.height(); ++row) {
          for (int col = 0; col < nimage.width(); ++col) {
            ...
            //setting the pixel color, not very readable at the moment
            c_image.setPixelColor(col, row, QColor(((col_r =="red_d")||!(allckbox.at(0)->isChecked()))? 0:r,
                                                   ((col_g =="green_d")||!(allckbox.at(1)->isChecked()))? 0:g,
                                                   ((col_b =="blue_d")||!(allckbox.at(2)->isChecked()))? 0:b));
          }
        }
